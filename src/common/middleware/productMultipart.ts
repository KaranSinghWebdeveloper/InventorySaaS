...
const getContentType = (headers: string) => {
...
export const parseProductMultipart = (req: Request, _res: Response, next: NextFunction) => {
  ...
  req.on("end", () => {
    try {
      ...
      for (const part of parts) {
        ...
        if (filename) {
          if (name !== "image" || !value.length) continue;

          const fileContentType = getContentType(rawHeaders);
          if (fileContentType && !allowedImageTypes.has(fileContentType)) {
            return next(new BadRequestError("Product image must be a JPEG, PNG, WebP, or GIF file"));
          }

          const fileName = `${Date.now()}-${crypto.randomUUID()}${safeExtension(filename)}`;
          const relativePath = path.posix.join("/uploads/product-images", fileName);
