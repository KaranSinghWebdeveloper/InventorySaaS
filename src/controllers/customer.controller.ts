...
export class CustomerController {
  ...
  getById = async (req: Request, res: Response, next: NextFunction) => {
    try {
      return sendSuccess(res, "Customer fetched", await this.customerService.getById(req.user!.businessId, Number(req.params.id)));
    } catch (error) {
      return next(error);
    }
  };
  ...
  update = async (req: Request, res: Response, next: NextFunction) => {
    try {
      return sendSuccess(res, "Customer updated", await this.customerService.update(req.user!.businessId, Number(req.params.id), req.body));
    } catch (error) {
      return next(error);
    }
  };
  ...
  delete = async (req: Request, res: Response, next: NextFunction) => {
    try {
      await this.customerService.delete(req.user!.businessId, Number(req.params.id));
